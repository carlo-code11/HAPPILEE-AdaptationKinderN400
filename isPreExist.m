...
preExist = questdlg('Load pre-existing set of input parameters?', ...
                        'Choose Option', ...
                        'yes', 'no', 'yes');
switch preExist
    case 'no'
...
%         end
        [paramFile, paramPath] = uigetfile('*.mat', 'Select your parameter file:','C:\Users\carlo\OneDrive\Documents\Uni2\Master\Masterarbeit\Code'); %auf Labor PC: R:\
%     end
...
%                 'parameter versions']) ; end
    proceed = questdlg('These parameters were saved using a different version of HAPPE and may be incompatible with the running pipeline. Proceed anyway?',...
                        'Choose Option', ...
                        'yes', 'no', 'yes');
    switch proceed
        case 'no'
    ...
    if ~proceed; error(['Mismatch between HAPPE and input ' ...
                 'parameter versions']) ; end
...
%     listParams(params) ;
    gui = makeParamGUI();      % Create GUI window
    listParamsGUI(gui, params) % Print your parameters into it
%     fprintf('Change an existing parameter? [Y/N]\n') ;
%     changeParams = choose2('n', 'y') ;
    changeParams = questdlg('Change an existing parameter?', ...
                        'Choose Option', ...
                        'yes', 'no', 'yes');
    switch changeParams
        case 'yes'
