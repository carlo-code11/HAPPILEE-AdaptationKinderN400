...
reprocessing = questdlg('Select an option: raw = Run on raw data from the start, reprocess = Run on HAPPE-processed data starting post-artifact reduction', ...
                        'Choose Option', ...
                        'raw', 'reprocess', 'raw');
switch reprocessing
    case 'raw'
...
%         'starts after muscIL (only available if muscIL-cleaned data exists)\n']) ;
    ...
    ranMuscIL = questdlg('Run from wavelet-cleaned data or muscIL-cleaned data? wavclean = Run starts after wavelet-cleaning, muscIL = Run starts after muscIL (only available if muscIL-cleaned data exists)', ...
                        'Choose Option', ...
                        'wavclean', 'muscIL', 'wavclean');
    switch ranMuscIL
        case 'wavclean'
            ranMuscIL = 0;
        case 'muscIL'
...
%     end
    ...
    overwrite = questdlg('Files, such as processed data and quality metrics may already exist for this dataset. Do you want to overwrite or save new files?', ...
                        'Choose Option', ...
    ...
    if overwrite 
        custom = questdlg('Use a default or custom label for processed data? default = Default name (_rerun_dd-mm-yyyy), custom = Create your own file label, starting with an underscore', ...
                        'Choose Option', ...
        ...
        if custom 
            ...
            rerunExt = inputdlg('Enter custom label:') ;
        end
    else
        rerunExt = '' ;
    end
