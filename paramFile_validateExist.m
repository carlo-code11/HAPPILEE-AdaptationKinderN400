...
%             end
%         end
          overwrite = questdlg('A set of input parameters with this name already exists.  overwrite = Overwrite the existing file,  new = Save a new file with a different name', ...
            'Choose Option', ...
          ...
          switch overwrite
              ...
              case 'new'
                  if indx == 0
                    new_name = inputdlg('Enter a new name for the file:') ;
                    paramFile = paramFile_validateExist(new_name{1}, ...
                        defaultName, 0) ;
                  else; paramFile = paramFile_validateExist([defaultName ...
                        datestr(now, 'dd-mm-yyyy') '_' num2str(indx) '.mat'], ...
                        defaultName, indx+1) ;
