...
%         '  4 = .cdt (Neuroscan)\n  5 = .mff (EGI)\n  6 = .edf\n' ...
%         '  7 = .bdf->.set (Mentalab)\n']) ;
    inputFormat = inputdlg('File format: 1 = .mat (MATLAB file), 2 = .raw (Net Station simple binary), 3 = .set (EEGLAB format), 4 = .cdt (Neuroscan), 5 = .mff (EGI), 6 = .edf, 7 = .bdf->.set (Mentalab)');
    inputFormat = str2num(inputFormat{1});
    if ~ismember(inputFormat, 1:7)
        disp("Invalid input: please enter an integer between 1 and 7.") ;
    end
%     while true
%         inputFormat = input('> ') ;
...
%     '\n  4 = Mentalab Explore\n  5 = Other\n']) ;
    inputFormat = inputdlg('Acquisition layout type: 1 = EGI Geodesic Sensor Net, 2 = EGI HydroCel Geodesic Sensor Net, 3 = Neuroscan Quik-Cap, 4 = Mentalab Explore, 5 = Other');
    inputFormat = str2num(inputFormat{1});
    if ~ismember(inputFormat, 1:5)
...
while true
    if inputFormat == 2
        if layout(1) == 1; msgbox('For .raw EGI GSN files, HAPPE supports 64 channels.') ;
        elseif layout(1) == 2; msgbox('For .raw EGI HydroCel GSN files, HAPPE supports 32, 64, 128, and 256 channels') ;
        else; error('HAPPE does not support this net for .raw files.') ;
        end
    elseif inputFormat == 4
        if layout(1) ~= 3; error('To run a .cdt file, the net must be a NeuroScan layout.') ; end
    elseif inputFormat == 5
        if layout(1) == 1; msgbox('For .mff EGI GSN, HAPPE supports 64 channels') ;
        elseif layout(1) == 2; msgbox('For .mff EGI HydroCel GSN, HAPPE supports 32, 64, 128, and 256 channels') ;
        else
...
%     layout(2) = input('> ') ;
    layout2 = inputdlg('Number of channels:');
    layout(2) = str2num(layout2{1});
    ...
    if inputFormat == 2 && ((layout(1) == 1 && layout(2) ~= 64) || ...
        ...
        msgbox('Invalid input: please enter a supported number of channels') ;
    else; break;
    end  
end
end
...
%         end
        eventLoc = uigetdir('R:\', 'Path to .txt files containing task event info:');
        if eventLoc ~= 0
            disp(['User selected folder: ', eventLoc]);
        else
...
%     inc = choose2('n', 'y') ;
    inc = questdlg('Do you have a channel locations file for your data?', ...
                'Choose Option', ...
                'yes', 'no', 'yes');
    switch inc
        case 'no'
...
%             'file extension.\n> '], 's') ;
        chanlocsFile = uigetfile('Select your channel locations file:','C:\');
    else
...
%                 'file.']) ;
%         end
        cont = questdlg('HAPPE functionality is limited without channel locations. Continue anyway?', ...
                    'Choose Option', ...
                    'yes', 'no', 'yes');
        switch cont
            case 'no'
                    error('User Termination: No channel locations for .mat file.') ;
            case 'yes'
                    chanlocsFile = [] ;
        end 
    end
